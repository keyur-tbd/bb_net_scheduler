# BigBasket GRN Scheduler

Pulls BigBasket **GRN** documents out of Gmail every 3 hours, extracts the line items, and writes them to Google Sheets and Supabase.
...
| Stage | What happens |
|---|---|
| **1. Gmail -> Drive** | Searches mail from `bbnet2@bigbasket.com` matching `grn`, looking back 2 days, and saves each attachment to a Drive folder. Already-seen files are skipped. |
| **2. Extract** | Each new file is parsed as **Excel/CSV attachments** with pandas (no LlamaParse). |
| **3. Sheets** | Rows are appended to tab `bbalertgrn_2` of the tracking spreadsheet. |
| **4. Supabase** | `supabase_sink.py --run --source bbnet` writes the same rows to table `bb_net_grn`. Any field without a typed column is preserved in `raw_data` (jsonb), so a renamed or new field never fails a run. |
...
python supabase_sink.py --run --source bbnet --limit 2
```
...
The extractor's output reaches Supabase whether or not a typed column exists. To promote a field: extend the `bbnet` entry in `SOURCES` in `supabase_sink.py`, run `--print-schema`, and apply the `alter table` it prints. Until then, query it as `raw_data->>'your_key'`.
